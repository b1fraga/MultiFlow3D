...
!##########################################################################
          ...
          real(dp) :: tr1,tr2,tr3,tr4,tr5,tr6,tr7,tr8,tr9
          real(dp) :: sp2,sp1,sm1,sm2
...
!!!!
                          ...
                          tr2=0.5_dp*(fi(i,j,k-1)+fi(i,j+1,k-1))
                          tr3=0.5_dp*(fi(i+1,j,k-1)+fi(i+1,j+1,k-1))
                          tr4=0.5_dp*(fi(i-1,j,k)+fi(i-1,j+1,k))
                          tr5=0.5_dp*(fi(i,j,k)+fi(i,j+1,k))
                          tr6=0.5_dp*(fi(i+1,j,k)+fi(i+1,j+1,k))
                          tr7=0.5_dp*(fi(i-1,j,k+1)+fi(i-1,j+1,k+1))
                          tr8=0.5_dp*(fi(i,j,k+1)+fi(i,j+1,k+1))
                          tr9=0.5_dp*(fi(i+1,j,k+1)+fi(i+1,j+1,k+1))
...
!     & +3.0d0*fi(i+1,j+1,k+1))
              ...
              do i=no1,nic+no2
                          ...
                          k1=2*k-pl-1
                          k2=2*k-pl
                          fic(ib,i,j,k)=0.25_dp*(fi(i1,jj,k1)+fi(i1,jj,k2)+ &
                                     fi(i2,jj,k1)+fi(i2,jj,k2))

!        h1=i1+2; h2=jj; h3=k1; sp2=(-fi(h1,h2,h3-1)+9.0_dp*fi(h1,h2,h3)
!     &+9.0_dp*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0_dp)
!        h1=i1+1; h2=jj; h3=k1; sp1=(-fi(h1,h2,h3-1)+9.0_dp*fi(h1,h2,h3)
!     &+9.0_dp*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0_dp)
!        h1=i1;   h2=jj; h3=k1; sm1=(-fi(h1,h2,h3-1)+9.0_dp*fi(h1,h2,h3)
!     &+9.0_dp*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0_dp)
!        h1=i1-1; h2=jj; h3=k1; sm2=(-fi(h1,h2,h3-1)+9.0_dp*fi(h1,h2,h3)
!     &+9.0_dp*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0_dp)
...
!==========================================================================
          do ly=0,nly
              ...
              do ib=1,nbp
                  ...
                  if (dom(ib)%jnext<0 .and. dom(ib)%bc_north/=5) jef=jef-1
                  isprf=pl+1
                  ieprf=nif-pl
                  jsprf=pl+1
                  jeprf=njf-pl
                  ...
                  if (dom(ib)%jnext<0 .and. dom(ib)%bc_north/=5) jec=jec-1
                  isprc=pl+1
                  ieprc=nic-pl
                  jsprc=pl+1
                  jeprc=njc-pl
...
!..........................................................................
            ...
            MPI_COMM_WORLD,dom(ib)%rq_m1,ierr)
                  ...
                  do i=no1,nic+no2
                          ...
                          h2=jj
                          h3=k1
                          sp2=(-fi(h1,h2,h3-1)+9.0_dp*fi(h1,h2,h3) &
                    +9.0_dp*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0_dp)
                          h1=i1+1
                          h2=jj
                          h3=k1
                          sp1=(-fi(h1,h2,h3-1)+9.0_dp*fi(h1,h2,h3) &
                    +9.0_dp*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0_dp)
                          h1=i1
                          h2=jj
                          h3=k1
                          sm1=(-fi(h1,h2,h3-1)+9.0_dp*fi(h1,h2,h3) &
                    +9.0_dp*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0_dp)
                          h1=i1-1
                          h2=jj
                          h3=k1
                          sm2=(-fi(h1,h2,h3-1)+9.0_dp*fi(h1,h2,h3) &
                    +9.0_dp*fi(h1,h2,h3+1)-fi(h1,h2,h3+2))/(16.0_dp)
...
!..............................................................................
                  ...
                  kepr=nk-pl
                  if(LMR==2 .and. &
            rdiv(dom_id(ib))<rdiv(dom(ib)%iprev)) jspr=pl
...
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgprev6)) jepr=nj-pl-1
            ...
            rdiv(dom_id(ib))>rdiv(dom(ib)%jprev)) then
                  do k=kspr,kepr
                  do i=ispr,iepr
                  ijk=(k-1)*ni+i
                          fi(i,js-1-ly,k)=( &
                    dom(ib) %recvb_m2(ijk)+3.0_dp*fi(i,js-ly,k)-fi(i,js+1-ly,k))/3.0_dp
                      end do
                      end do
                  else
                  do k=kspr,kepr
                  ...
                  kepr=nk-pl
                  if(LMR==2 .and. &
            rdiv(dom_id(ib))<rdiv(dom(ib)%kprev)) jspr=pl
...
!..............................................................................
                  ...
                  kepr=nk-pl
                  if(LMR==2 .and. &
            rdiv(dom_id(ib))<rdiv(dom(ib)%inext)) jspr=pl
...
!     & rdiv(dom_id(ib)).eq.rdiv(dom(ib)%edgnext5)) jepr=nj-pl-1
                  ...
                  kepr=nk-pl
                  if(ly==0 .and. &
            rdiv(dom_id(ib))>rdiv(dom(ib)%jnext)) then
                  do k=kspr,kepr
                  do i=ispr,iepr
                  ijk=(k-1)*ni+i
                          fi(i,je+1+ly,k)=( &
                    dom(ib) %recvb_p2(ijk)+3.0_dp*fi(i,je+ly,k)-fi(i,je+ly-1,k))/3.0_dp
                      end do
                      end do
                  else
                  do k=kspr,kepr
                  ...
                  kepr=nk-pl
                  if(LMR==2 .and. &
            rdiv(dom_id(ib))<rdiv(dom(ib)%knext)) jspr=pl
